...
int Partition (int *array, int lowB, int highB)
{
    ...
    {
        while(array[leftInt] <= array[pivot])   //Check before submitting
        {
            leftInt++;
            NumOfComparisons++; 
        }
        while (array[rightInt] > array[pivot])
        {
            rightInt--; 
            NumOfComparisons++;
        }  
        ...
        {
            if (array[leftInt] == array[rightInt])
            {
                Swap(&array[leftInt + 1], &array[rightInt]); 
                NumOfComparisons++; 
            }
